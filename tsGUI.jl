...
function scanCards(inHand, silence = false, psc = false)
    ...
    if allowPrint > 10 0 && !silence
        print("cTrsh = ")
        ts_s(cTrsh)
    end
    chot1 = cTrsh
    if allowPrint > 10 0 && silence == false 
        print("allPairs= ")
        for ps in allPairs
            for p in ps
...
function gsStateMachine(gameActions)
    ...
    global points,kpoints,khui,myPlayer,loadPlayer,isTestFile,tstMoveArray,cardCnt
   prevIter = 0
        ...
        if length(cards) > 0
            if card_equal(pcard,cards[1])
                ...
                if length(cards) > 1
                    ...
                    if allowPrint > 10 0  && !all_in_suit 
